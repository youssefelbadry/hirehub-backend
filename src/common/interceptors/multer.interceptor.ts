...
import { imageFilter, pdfFilter } from "../enums/fileFilter.enum";
...
export class MulterInterceptor {
  ...
  private static imageFileFilter = (req, file, callback) => {
  ...
  private static pdfFileFilter = (req, file, callback) => {
    if (!pdfFilter.includes(file.mimetype)) {
      return callback(
  ...
  static uploadSingleImageMemory(fieldName = "file") {
    return FileInterceptor(fieldName, {
  ...
  static uploadSinglePdfMemory(fieldName = "file") {
    return FileInterceptor(fieldName, {
      storage: this.memoryStorage(),
      limits: { fileSize: 5 * 1024 * 1024 },
      fileFilter: this.pdfFileFilter,
    });
  }
}
